             ...
             GError **err)
{
    ...
    if (g_cancellable_is_cancelled (cancellable))
        {
            g_set_error_literal (
                err, NBT_GLIB_PARSE_ERROR, NBT_GLIB_PARSE_ERROR_CANCELLED,
    ...
    switch (tag)
        ...
        array_error:
            ...
            return 1;
        case_default:
            g_set_error (err, NBT_GLIB_PARSE_ERROR,
                         NBT_GLIB_PARSE_ERROR_INTERRUPTED,
    ...
    for (; result != G_CONVERTER_FINISHED;)
        {
            ...
            result = g_converter_convert (
                ...
                G_CONVERTER_NO_FLAGS, &current_consumed_len,
                &current_write_len, &internal_err);
            if (set_func && klass)
                {
                    clock_t passed_ms
                    ...
                    if (passed_ms % 500 == 0)
                        set_func (klass,
                                  (original_len - length) * 100 / original_len,
                                  _ ("Decompressing."));
                }
